...
> **Note d'exactitude (2026-05-29, post-audit v0.1.2-pre)** : le bloc « Décision » ci-dessus est *historique* — l'implémentation livrée diffère sur deux points. (1) Les sentinels exportés sont des `errors.New(...)` portant la chaîne `Diagnostic` exacte (ex. `"hors frontière τ"`), et non des `&RefusError{Diagnostic: "frontiere_franchie"}` ; ils correspondent verbatim aux constantes `tau.Diag*` (`internal/tau/diagnostics.go`). (2) **Q5-01 RÉSOLU** *(2026-05-29)* : `*RefusError` implémente désormais `func (e *RefusError) Is(target error) bool` qui compare `e.Diagnostic` à `target.Error()`. En conséquence, un `*RefusError` *produit à l'exécution* « matche » bien le sentinel correspondant via `errors.Is` (ex. `errors.Is(refus, ErrFrontiereFranchie) == true` dès que `refus.Diagnostic == ErrFrontiereFranchie.Error()`) — le matching ne dépend plus de l'identité de pointeur. La ligne « RefusError has no Unwrap » reste exacte (un refus est terminal, sans cause encapsulée) ; le matching passe par `Is`, pas par `Unwrap`. Garde de régression : `TestRefusError_Is_MatchesDiagnosticSentinel` (`internal/errors/errors_test.go`). Voir [`docs/archive/audits/2026-05-29-AUDIT-v0.1.2-pre/`](../archive/audits/2026-05-29-AUDIT-v0.1.2-pre/).
